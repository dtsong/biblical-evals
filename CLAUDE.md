...
```
.env.example     ← Checked in — documents all variables
.env             ← NOT checked in — your local overrides (root source of truth)
apps/api/.env.local  ← Generated by setup-env.sh
apps/web/.env.local  ← Generated by setup-env.sh
```

- **Setup**: `cp .env.example .env`, fill in values, run `./scripts/setup-env.sh`
- **Resolution order**: Root `.env` values take priority, GCP Secret Manager fills gaps
